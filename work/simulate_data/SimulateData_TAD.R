...
for (iteration in (1:5)[1]) {
  for (noisePercentage in c(0.04, 0.08, 0.12, 0.16, 0.20)) {
    ...
    check<-runHiCSimulation(BINSIZE=40000, simChromName="chr5", max.tad=50, min.tad=3,
     TARGET.chrsize=180.92*1e6, base.tad.signal=28, interaction.decay.rate=0.69,
     percent_of_sampling_points_nonSpecificsignal=noisePercentage, dispersion=0.01, PNGplot=FALSE, saveVanilla=FALSE,
     saveDiffHiC=FALSE, saveHOMER=FALSE, addIntConstant=FALSE,nestedTADs=FALSE)
